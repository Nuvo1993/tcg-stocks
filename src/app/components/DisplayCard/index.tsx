        ...
        <div className="grid-cols-1 ml-2 mb-2">
          ...
          {user && !isProfilePage ? (
            <div className="flex justify-between">
              <button
                onClick={async () => await addCardToUserCollection(card)}
                className="grow bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center mr-1 sm:mr-0 2xl:mr-1"
              >
                <svg
              ...
              <button
                onClick={async () => await addCardToWishlist(card)}
                className="grow bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center mt-0 sm:mt-1 md:mt-1 lg:mt-1 2xl:mt-0"
              >
                <svg
                  className="svg-icon w-6 h-6"
                  viewBox="0 0 20 20"
            ...
            <div className="flex justify-between">
            <button
              onClick={async () => await removeCardFromUserCollection(card.docId)}
              className="grow bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center mr-1 sm:mr-0 2xl:mr-1"
            >
              <svg
                className="svg-icon w-6 h-6"
            ...
            <button
              onClick={async () => await removeCardFromWishlist(card.docId)}
              className="grow bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center mt-0 sm:mt-1 md:mt-1 lg:mt-1 2xl:mt-0"
            >
              <svg
                className="svg-icon w-6 h-6"
